...
$sql = "SELECT s.service_no, s.service_name, s.service_industry, s.service_state, s.service_localg, s.service_streetA, pt.product_no, ptd.p_description_Name FROM service AS s Join producttype As pt ON s.service_no= pt.product_parent Join producttype_description As ptd ON pt.product_no = ptd.p_description_id  WHERE (";
for($i = 0; $i < $p; $i++) {
...
while($row = mysql_fetch_array($rs)) {
  ...
  $a_json_row["label"] = $row[1]." (".$row[2].') '.$row[5].", ".$row[4].", ".$row[3];
  $a_json_row["n"] = $row[1];
  $a_json_row["i"] = $row[2];
  $a_json_row["a"] = $row[5];
  $a_json_row["a2"] = $row[4];
...
$a_json_row["id"] = 0;
  ...
  $a_json_row["label"] = 'Add New Service';
  array_push($a_json, $a_json_row);
